...
### ~~3.5 validate-commit-msg~~

[validate-commit-msg](https://github.com/kentcdodds/validate-commit-msg?fileGuid=NOAdP6yyJZtzFyAP)用于检查 Node 项目的 Commit message 是否符合格式。
...
```
接着，把这个脚本运行加入 Git 的 hook（需要 安装 ghook）。下面是在package.json里面使用[ghooks](http://npm.im/ghooks?fileGuid=NOAdP6yyJZtzFyAP)，把这个脚本加为commit-msg时运行。
```
 // 安装 ghooks
 npm install ghooks --save-dev
 //package.json: 只列出了 validate-commit-msg 自动验证相关配置项
    "scripts": {
